...
class AvatarSnapshotTest {
    ...
    @Test
    fun snapshotEmoji_Felipe() = assertVariantSnapshot(AvatarVariant.EMOJI, "Felipe", maxDiff = 15.0)

    @Test
    fun snapshotEmoji_Maria() = assertVariantSnapshot(AvatarVariant.EMOJI, "Maria", maxDiff = 15.0)

    @Test
    fun snapshotEmoji_Steve() = assertVariantSnapshot(AvatarVariant.EMOJI, "Steve", maxDiff = 15.0)
    ...
    private fun assertVariantSnapshot(
        ...
        val image = onRoot().captureToImage()
        SnapshotTestUtil.assertSnapshot("${variant.name.lowercase()}_$name", image, maxPixelDiffPercent = maxDiff)
    }
}
